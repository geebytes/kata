...
import { checkFreshness } from './evidence.js';
...
export async function judge(input) {
    ...
    const freshEvidence = input.evidence.filter((evidence) => checkFreshness(evidence, input.currentDiffHash, input.currentScopeHashes?.get(evidence.id)).fresh);
