...
export async function initLlmWiki(input) {
    ...
    for (const name of ['AGENTS.md']) {
        ...
        try {
            await stat(rootPath);
            if (!sourceFiles.includes(rootPath))
...
export async function orientLlmWiki(input = {}) {
    const root = resolve(input.root ?? process.cwd());
    const wikiPath = normalizeWikiPath(input.wikiPath);
    const wikiRoot = resolveWikiRoot(root, wikiPath);
...
export async function ingestLlmWiki(input) {
    const root = resolve(input.root ?? process.cwd());
    const wikiPath = normalizeWikiPath(input.wikiPath);
    const wikiRoot = resolveWikiRoot(root, wikiPath);
...
export async function queryLlmWiki(input) {
    const root = resolve(input.root ?? process.cwd());
    const wikiPath = normalizeWikiPath(input.wikiPath);
    const wikiRoot = resolveWikiRoot(root, wikiPath);
...
export async function buildLlmWikiTask(input) {
    const root = resolve(input.root ?? process.cwd());
    const wikiPath = normalizeWikiPath(input.wikiPath);
    const wikiRoot = resolveWikiRoot(root, wikiPath);
...
const wikiPageDirectories = ['concepts', 'entities', 'comparisons', 'queries'];
export async function registerWikiPages(input = {}) {
    const root = resolve(input.root ?? process.cwd());
    const wikiPath = normalizeWikiPath(input.wikiPath);
    const wikiRoot = resolveWikiRoot(root, wikiPath);
...
export async function rebuildLlmWiki(input = {}) {
    const root = resolve(input.root ?? process.cwd());
    const wikiPath = normalizeWikiPath(input.wikiPath);
    const wikiRoot = resolveWikiRoot(root, wikiPath);
    let cleanedPages = 0;
...
export async function lintLlmWiki(input = {}) {
    const root = resolve(input.root ?? process.cwd());
    const wikiPath = normalizeWikiPath(input.wikiPath);
    const wikiRoot = resolveWikiRoot(root, wikiPath);
    const issues = [];
    ...
    for (const page of wikiPages)
        inbound.set(page, index.includes(`[[${page}]]`) ? 1 : 0);
    for (const page of wikiPages) {
...
function normalizeTaskRead(root, wikiRoot, wikiPath, sourceFile) {
    ...
    if (!sourcePath.startsWith('..') && !isAbsolute(sourcePath)) {
        if (sourcePath.startsWith(`${wikiPath}/`))
            return sourcePath;
        const wikiRelative = relative(wikiRoot, sourceFile).replaceAll('\\', '/');
        if (!wikiRelative.startsWith('..'))
            return `${wikiPath}/${wikiRelative}`;
        return sourcePath;
    }
    const wikiRelative = relative(wikiRoot, sourceFile).replaceAll('\\', '/');
...
function renderQueryAnswer(query, matches) {
    if (matches.length === 0)
        return `No compiled LLM Wiki pages matched "${query}".`;
...
function resolveWikiLink(link, existingPages) {
    ...
    for (const page of existingPages) {
        if (page === basename || page.endsWith(`/${basename}`) || page.endsWith(`/${basename}.md`))
            return page;
    }
    return null;
}
