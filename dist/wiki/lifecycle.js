...
export async function auditWiki(root) {
    ...
    for (const record of recordsBeforeDrift.filter((item) => item.status === 'candidate'))
        candidatesByTask.set(record.validationTaskId, (candidatesByTask.get(record.validationTaskId) ?? 0) + 1);
...
export async function createRefreshPacket(root, taskId) {
...
export async function relevantWiki(root, taskId, limit = 8) {
...
function score(record, terms) { const text = `${record.statement} ${record.scope.join(' ')} ${record.sourceRefs.join(' ')}`.toLowerCase(); return [...terms].reduce((total, term) => total + (text.includes(term) ? 1 : 0), 0); }
async function countPages(root) { try {
    const entries = await readdir(root, { withFileTypes: true });
    const counts = await Promise.all(entries.map((entry) => entry.isDirectory() ? countPages(join(root, entry.name)) : entry.name.endsWith('.md') ? 1 : 0));
...
function lifecycleActions(records, context) {
    ...
    for (const record of records.filter((item) => item.status !== 'rejected')) {
        ...
        if (reviewDueIds.has(record.id))
            reasons.push('review_due');
        const originalStatus = context.originalStatusById.get(record.id) ?? record.status;
        if (originalStatus === 'candidate' && context.overBudgetTaskIds.has(record.validationTaskId))
            reasons.push('candidate_over_budget');
        ...
        if (successors.length > 0)
            reasons.push('semantic_superseded');
...
function chooseAction(record, reasons, successors) {
    if (reasons.includes('semantic_superseded') && successors.length > 0)
        return 'retire';
    if (reasons.includes('conflict'))
        return 'review_conflict';
    if (reasons.includes('source_missing') && record.status !== 'verified')
        return 'retire';
    if (reasons.includes('duplicate') && record.status === 'candidate')
        return reasons.includes('candidate_over_budget') ? 'merge' : 'retire_duplicate';
    if (reasons.includes('source_changed') || reasons.includes('scope_changed'))
        return 'revalidate';
    return 'review';
}
function confidence(reasons, successors) {
    if ((reasons.includes('source_missing') || reasons.includes('semantic_superseded')) && successors.length > 0)
        return 'high';
    if (reasons.includes('source_changed') || reasons.includes('duplicate'))
        return 'medium';
    return 'low';
}
