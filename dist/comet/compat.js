...
export function loadCometCompatibility(manifestPath = new URL('../../comet-compat.yaml', import.meta.url)) {
    ...
    if (!minVersion)
        throw new Error('comet-compat.yaml must declare comet.minVersion');
...
function parseVersion(version) {
    ...
    if (!match)
        throw new Error(`Invalid Comet version: ${version}`);
...
export function assertCometVersion(version, compatibility) {
...
export function assertCapability(compatibility, capability) {
...
function readIndentedBlock(manifest, key) {
    ...
    if (start === -1)
        throw new Error(`comet-compat.yaml is missing ${key} block`);
...
function readScalar(block, key) {
    ...
    if (!value)
        throw new Error(`comet-compat.yaml is missing ${key}`);
    return value;
}
