...
import { appendStateEvent, transition, writeCurrentState } from '../core/state.js';
import { buildContextManifest } from '../core/context.js';
import { checkFreshness, collectEvidence, computeDiffHash } from '../quality/evidence.js';
...
import { acknowledgeCometOpen, defaultWorkflowProfile, isWorkflowProfile } from '../core/workflow-profile.js';
...
import { createTaskRevision, findOwnershipConflicts, readTaskRevision, revisionStatus, workspaceDrift } from './revision.js';
import { classifyCodeGraphCandidates, discoverCodeGraphCandidates, readWaivers, validateMatrix, validatePathCoverage, validateWaivers, writeWaivers, requiresMatrix, getMatrixRowForAc, evidenceMatchesRow } from '../quality/acceptance-matrix.js';
import { readObligations, persistBlockingJudgeResult, resolveObligationsForRevision } from '../quality/repair-obligations.js';
const defaultActor = { id: 'kata-agent', role: 'implementer' };
const reviewerActor = { id: 'kata-reviewer', role: 'reviewer' };
const judgeActor = { id: 'kata-judge', role: 'judge' };
...
export async function runCommand(command, taskId, root, options = {}) {
...
async function cmdDesign(taskId, root, options) {
    ...
    const current = JSON.parse(await readFile(join(root, '.kata/tasks', taskId, 'current-state.json'), 'utf8'));
...
async function acknowledgeCometOpenIfRequired(root, taskId) {
    ...
    if (!isWorkflowProfile(task.workflowProfile))
        return undefined;
    if (task.workflowProfile.comet.openStatus !== 'required')
        return task.workflowProfile;
...
async function cmdBuild(taskId, root, options = {}) {
    ...
    if (!(options.seal ?? true)) {
        let buildOwnedPaths = [];
        try {
            const currentTask = JSON.parse(await readFile(join(root, '.kata/tasks', taskId, 'task.json'), 'utf8'));
    ...
    if (revision && task.acceptanceMatrix) {
        ...
        await resolveObligationsForRevision(root, taskId, revision.id, acIds, evidence.filter((e) => e.exitCode === 0).map((e) => e.id), task.acceptanceMatrix, evidence);
    }
...
function matrixChecks(root, matrix) {
    ...
    return checks.filter((check, index) => checks.findIndex((candidate) => candidate.kind === check.kind
        && candidate.command === check.command
        && (candidate.args ?? []).join('\0') === (check.args ?? []).join('\0')
        && candidate.cwd === check.cwd) === index);
}
...
async function persistTaskOwnedPaths(root, taskId, task, ownedPaths) {
    await writeFile(join(root, '.kata/tasks', taskId, 'task.json'), `${JSON.stringify({ ...task, ownedPaths }, null, 2)}\n`, 'utf8');
}
async function writeEvidence(root, taskId, evidence) {
    ...
    for (const envelope of evidence) {
        await writeFile(join(evidenceDir, `${taskId}-${evidenceFileSuffix(envelope)}.json`), `${JSON.stringify(envelope, null, 2)}\n`, 'utf8');
    }
    const testEvidence = evidence.find((item) => item.kind === 'test');
    if (testEvidence) {
        await writeFile(join(evidenceDir, `${taskId}-hard.json`), `${JSON.stringify(testEvidence, null, 2)}\n`, 'utf8');
    }
}
...
async function reenterImplementForReviewRepair(taskId, root, actor) {
    ...
    await writeFile(join(root, '.kata/tasks', taskId, 'repair.json'), `${JSON.stringify({
        taskId,
        fromPhase: 'review',
        toPhase: 'implement',
        actor,
...
async function cmdVerify(taskId, root, options = {}) {
    ...
    if (revisionId)
        verifyResult.revisionId = revisionId;
...
async function cmdJudge(taskId, root, options = {}) {
    ...
    if (judgeResult.result === 'FAIL') {
        await persistBlockingJudgeResult(root, taskId, judgeResult.acceptance.filter((a) => a.result === 'FAIL').map((a) => ({ id: a.id, result: a.result })));
    }
...
async function readReviewFindings(root, taskId) {
...
function evaluateReadiness(taskId, acceptance, evidence, findings, currentDiffHash, scopeHashes, matrix, unresolvedObligations = [], reviewMode) {
    ...
    const acceptanceResults = acceptance.map((criterion) => {
        ...
        if (obligation)
            return { id: acceptanceId, result: 'FAIL', repairScope: 'unresolved_repair_obligation' };
        ...
        if (matrix) {
            ...
            if (row && (row.verificationLevel === 'integration' || row.verificationLevel === 'entrypoint')) {
                ...
                if (!hasRowSpecificEvidence)
                    return { id: acceptanceId, result: 'FAIL', repairScope: 'insufficient_evidence_level' };
            }
        }
        if (blockingFinding)
...
function revisionIdForEvidence(evidence) {
    ...
    if (revisionIds.length > 1)
        throw new Error('Evidence from multiple task revisions cannot be verified together');
...
async function currentScopeHashes(root, evidence) {
    ...
    return new Map(await Promise.all(evidence
        ...
        .map(async (item) => [item.id, await computeScopeHash(root, item.scope?.paths ?? [])])));
}
async function cmdArchive(taskId, root, options = {}) {
    ...
    const wikiClosure = await evaluateWikiClosure(root, taskId);
    if (!wikiClosure.valid) {
        const latest = JSON.parse(await readFile(join(root, '.kata/tasks', taskId, 'current-state.json'), 'utf8'));
        return {
            command: 'archive',
            taskId,
            phase: latest.phase,
