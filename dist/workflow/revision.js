...
export async function createTaskRevision(input) {
    ...
    if (ownedPaths.length === 0)
        throw new Error('A revision requires at least one declared owned path');
...
export async function readTaskRevision(root, taskId, revisionId) {
    return JSON.parse(await readFile(join(root, '.kata/tasks', taskId, 'revisions', `${revisionId}.json`), 'utf8'));
}
export async function readCurrentTaskRevision(root, taskId) {
    try {
        return JSON.parse(await readFile(join(root, '.kata/tasks', taskId, 'current-revision.json'), 'utf8'));
    }
    catch (error) {
...
export async function computeManifestHash(root, ownedPaths) {
...
export async function findOwnershipConflicts(root, taskId, ownedPaths) {
    ...
    for (const otherTaskId of entries.filter((id) => id !== taskId)) {
        try {
            ...
            if (terminal.taskId === taskId)
                continue;
            if (terminal.redirects.length > 0)
                continue;
            try {
                const state = JSON.parse(await readFile(join(tasksRoot, terminal.taskId, 'current-state.json'), 'utf8'));
                if (state.phase === 'archive')
                    continue;
            }
            catch { /* legacy task without state remains an active ownership claim */ }
            const task = JSON.parse(await readFile(join(tasksRoot, otherTaskId, 'task.json'), 'utf8'));
            for (const path of normalizeOwnedPaths(root, task.ownedPaths ?? [])) {
                if (normalized.some((owned) => pathsOverlap(owned, path)))
                    conflicts.push({ taskId: otherTaskId, path });
            }
        }
...
export async function workspaceDrift(root, ownedPaths) {
...
export async function inferOwnedPathsFromWorkspace(root) {
    return changedRepositoryPaths(root)
        .filter((path) => !isIgnoredWorkspacePath(path))
        .sort();
}
