...
import { loadCometCompatibility } from './comet/compat.js';
...
import { discoverPlatforms, identifyPlatformInstallState, install, listManagedPlatforms, uninstall, update, } from './adapters/discovery.js';
...
import { validateWaivers } from './quality/acceptance-matrix.js';
import { createWorkflowHandoff, renderDelegationPrompt } from './workflow/delegation-prompt.js';
import { createHandoff } from './workflow/handoff.js';
...
import { nextActionForTask, nextSkillForPhase, readUpstreamSummary, statusActionPrompts, suggestCandidateAction, } from './workflow/navigation.js';
...
import { activateHookTask, currentGitBranch, deactivateHookTask, readActiveHookTask } from './hooks/runtime.js';
import { platformDefinitionById } from './adapters/platforms.js';
import { doctor } from './adapters/doctor.js';
import { acknowledgeCometOpen, defaultWorkflowProfile, developmentModes, isolationModes, isWorkflowProfile, reviewModes, updateGitFlowProfile, } from './core/workflow-profile.js';
import { applyGitFlowPlan, inspectGitFlow } from './core/git-flow.js';
import { addKataRelation, addTaskRelation, findKataRelations, readTaskRelations, resolveTerminalTask, } from './core/relations.js';
export function getRuntimeCompatibility(manifestPath) {
...
async function runMain(argv) {
    ...
    if (isInstallerCommand(command) && (maybeChange === undefined || maybeChange.startsWith('--'))) {
        ...
        if (!args.options.root)
            args.options.root = workspaceRoot;
        if (command === 'uninstall') {
            const platformLabel = args.platform === 'generic' ? 'all platforms' : args.platform;
            const confirmed = args.options.force ?? await confirmDestructive(`Uninstall Kata components from platform: ${platformLabel}`, ['Removes hooks, rules, skills, and related files.', 'Cannot be undone automatically.']);
            if (!confirmed) {
    ...
    if (!change) {
        throw new Error('Usage: kata <init|update|uninstall|discover|comet|codegraph|status|open|design|build|verify|archive|hotfix|tweak|collect|next> [change|--change change]');
    }
    if (command === 'status') {
    ...
    if (command === 'init') {
        const initLanguage = process.stdin.isTTY
            ? await (await import('./cli/prompt.js')).select('Language for skills', [
        ...
        outputResult(await client.next(change));
...
async function runAggregateUpdate(scope, options) {
    ...
    const realPlatforms = [...new Set([...managed, ...detected])]
        .filter((platform) => platform !== 'generic')
        .sort();
    const targets = [...realPlatforms];
    if (targets.length === 0 || managed.includes('generic'))
...
function shouldUseInitWizard(argv) {
    if (argv.includes('--platform') || argv.includes('--scope') || argv.includes('--home'))
        return false;
...
async function runInitWizardCommand(argv, defaultRoot) {
    ...
    for (const platform of plan.selected) {
        reports.push(await install(platform.platform, plan.scope, optionsForWizardInstall(args.options, plan.scope, platform.root, plan.language)));
    }
    let codegraphResult = {
...
async function readWaiversFile(argv) {
    ...
    if (!path)
        throw new Error('Invalid waivers file: --waivers-file requires a path.');
    let parsed;
    try {
    ...
    if (errors.length > 0)
        throw new Error(`Invalid waivers file: ${errors.join('; ')}`);
    return waivers;
}
async function runGitFlowCommand(argv, root) {
    if (argv[0] !== 'apply')
        throw new Error('Usage: kata git-flow apply --change <task-id>');
    ...
    return {
        ...
        nextAction: state.status === 'active'
            ...
            : { cliCommand: `kata git-flow apply --change ${taskId} --confirm`, reason: inspected.reason ?? 'git_flow_setup_failed' },
    };
}
...
function parseEnumArg(argv, flags, allowed, label) {
    ...
    if (!value)
        throw new Error(`Missing ${label} after ${flag}`);
...
async function discoverSingleTaskForCurrentBranch(root) {
    ...
    for (const taskId of taskIds) {
        try {
            const task = JSON.parse(await readFile(join(root, '.kata/tasks', taskId, 'task.json'), 'utf8'));
            if (task.branch === branch && task.phase !== 'archive')
                matches.push(task.id ?? taskId);
        }
        catch {
...
async function runLocalStatusCommand(change, resolved, root = resolveWorkspaceRoot()) {
    ...
    const state = JSON.parse(await readFile(join(root, '.kata/tasks', change, 'current-state.json'), 'utf8'));
...
async function runWikiCommand(argv) {
    ...
    if (subcommand === 'init') {
        if (!args.from)
            throw new Error('Usage: kata wiki init --from <path> [--wiki <path>] [--root <path>]');
    ...
    if (subcommand === 'ingest') {
        if (!args.from)
            throw new Error('Usage: kata wiki ingest --from <path> [--wiki <path>] [--root <path>]');
    ...
    if (subcommand === 'query') {
        if (!args.query)
            throw new Error('Usage: kata wiki query --q <question> [--file] [--wiki <path>] [--root <path>]');
    ...
    if (subcommand === 'audit')
        return { command: 'wiki audit', ...(await auditWiki(args.root ?? resolveWorkspaceRoot())) };
    if (subcommand === 'lifecycle')
        return { command: 'wiki lifecycle', ...(await auditWiki(args.root ?? resolveWorkspaceRoot())) };
    if (subcommand === 'refresh') {
        if (!args.task)
            throw new Error('Usage: kata wiki refresh --task <task-id> [--root <path>]');
        return { command: 'wiki refresh', ...(await createRefreshPacket(args.root ?? resolveWorkspaceRoot(), args.task)) };
    }
    if (subcommand === 'relevance') {
        if (!args.task)
            throw new Error('Usage: kata wiki relevance --task <task-id> [--root <path>]');
    ...
    if (subcommand === 'rebuild') {
        const confirmed = args.force ?? await confirmDestructive('This will remove all existing wiki pages and governed records, then regenerate the enrich task-packet.', ['Concepts, entities, comparisons, and queries will be deleted.', 'Governed candidate records in .kata/wiki/ will be deleted.']);
        if (!confirmed) {
...
async function runTasksCommand(argv) {
    ...
    if (subcommand === 'relations' || subcommand === 'show') {
        if (!args.task && !args.from)
            throw new Error('Usage: kata tasks relations --task <task> [--root <path>]');
...
async function runRelationsCommand(argv) {
    ...
    if (subcommand === 'show' || subcommand === 'list') {
        if (!args.id)
            throw new Error('Usage: kata relations show --id <task:id|change:id> [--root <path>]');
...
async function runHandoffCommand(argv) {
    ...
    const root = args.root ?? resolveWorkspaceRoot();
    if (!args.task)
        throw new Error('Usage: kata handoff <create|show|verify|acknowledge> --task <id>');
    if (subcommand === 'create') {
        if (!args.from || !args.to)
            throw new Error('Usage: kata handoff create --task <id> --from <role> --to <role>');
    ...
    if (!args.id)
        throw new Error('Usage: kata handoff <show|verify|acknowledge> --task <id> --id <handoff-id>');
    if (subcommand === 'show')
        return { command: 'handoff show', packet: await readContextPacket(root, args.task, args.id) };
    if (subcommand === 'verify')
        return { command: 'handoff verify', ...(await verifyContextPacket({ root, taskId: args.task, id: args.id })) };
    if (subcommand === 'acknowledge') {
        if (!args.platform || !args.role)
            throw new Error('Usage: kata handoff acknowledge --task <id> --id <handoff-id> --platform <name> --role <role>');
...
function createPacketHash(packet) { return createHash('sha256').update(JSON.stringify(packet)).digest('hex'); }
async function runDelegateCommand(argv) {
...
function parseDelegationArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (key?.startsWith('--'))
            throw new Error(`Unknown delegation option: ${key}`);
        if (!args.change)
            args.change = key;
    }
    return args;
...
async function listTaskCandidates(root) {
    ...
    for (const entry of entries.sort()) {
        ...
        if (!candidate)
            continue;
        if (branch && candidate.branch && candidate.branch !== branch)
            continue;
...
function inferDelegationRole(phase) {
    if (phase === 'hardVerify')
        return 'reviewer';
    if (phase === 'review')
        return 'judge';
...
function inferCurrentRole(phase) {
    if (phase === 'hardVerify')
        return 'implementer';
    if (phase === 'review')
        return 'reviewer';
...
function recommendDelegationTask(candidates) {
...
function recommendPlatform(platforms, role) {
...
function parseHandoffArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (!target || value === undefined)
            throw new Error(`Unknown handoff option: ${key}`);
        args[target] = value;
        index += 1;
    }
    return args;
...
async function runHooksCommand(argv) {
    ...
    if (subcommand === 'activate') {
        if (!args.change)
            throw new Error('Usage: kata hooks activate --change <id> [--role <role>] [--platform <name>] [--root <path>]');
        const active = await activateHookTask({
            root,
            taskId: args.change,
...
async function runDoctorCommand(argv) {
    ...
    if (hasExplicitPlatform)
        return doctor(args.platform, args.scope, args.options);
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === '--change' && value !== undefined) {
            ...
            if (!Number.isInteger(parsed) || parsed < 0)
                throw new Error(`Invalid failure count: ${value}`);
...
function parseRelationEndpoint(value) {
    ...
    if ((type === 'task' || type === 'change') && id.length > 0)
        return { type, id };
...
const CODEGRAPH_SUBCOMMANDS = ['explore', 'query', 'impact', 'affected', 'node', 'status', 'index', 'sync'];
...
async function runCometCommand(argv, root = resolveWorkspaceRoot()) {
    ...
    if (subcommand === 'acknowledge-open') {
        if (!args.change)
            throw new Error('Usage: kata comet acknowledge-open --change <id>');
...
function parsePlatform(value) {
    if (value in platformDefinitionById)
        return value;
