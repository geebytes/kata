...
import { skillCommands as allSkillCommands, } from './manifest.js';
...
export async function discoverPlatforms(options = {}) {
    ...
    for (const platform of platformDefinitions) {
        if (platform.id === 'generic')
            continue;
        if (await isDetected(platform.id, 'project', root))
            detected.push(platformInfo(platform.id, 'project', true, root));
        if (await isDetected(platform.id, 'global', home))
            detected.push(platformInfo(platform.id, 'global', true, home));
    }
...
async function isDetected(platform, scope, root) {
    ...
    if (platform === 'claude-code' && scope === 'global')
        paths.push('.claude.json');
...
export async function identifyPlatformInstallState(platform, options = {}) {
