...
import { commandManifest, renderSkill, skillCommands, } from './manifest.js';
import { platformCommandPath, platformDefinitionById, platformRulePath, platformSkillPath, platformSkillsDir } from './platforms.js';
export async function install(platform, scope, options = {}) {
    return writeSkills(platform, scope, options);
}
export async function update(platform, scope, options = {}) {
    return writeSkills(platform, scope, options);
}
export async function listManagedPlatforms(scope, options = {}) {
    ...
    return [...new Set(Object.values(manifest.files)
            ...
            .map((file) => file.platform))].sort();
}
export async function uninstall(platform, scope, options = {}) {
...
async function removeObsoleteCommandFiles(platform, scope, options, baseRoot, manifest, report) {
    ...
    const obsolete = Object.values(manifest.files).filter((file) => {
        if (file.platform !== platform || file.scope !== scope)
            return false;
    ...
    for (const file of obsolete) {
        ...
        if (!options.dryRun && current !== undefined)
            await rm(path);
        if (!options.dryRun)
...
function removeManagedHookConfig(content, commandId) {
    ...
    if (!parsed)
        return content;
    if (format === 'claude-code')
        return stringifyJson(removeGroupedHookConfig(parsed, 'PreToolUse'));
    if (format === 'gemini')
        return stringifyJson(removeGroupedHookConfig(parsed, 'BeforeTool'));
    if (format === 'windsurf')
        return stringifyJson(removeArrayHookConfig(parsed, 'pre_write_code'));
    if (format === 'copilot')
        return '';
    return content;
}
...
function removeGroupedHookConfig(existing, groupName) {
    ...
    if (filtered.length > 0)
        hooks[groupName] = filtered;
    else
        delete hooks[groupName];
...
function removeArrayHookConfig(existing, arrayName) {
    ...
    if (filtered.length > 0)
        hooks[arrayName] = filtered;
    else
        delete hooks[arrayName];
    if (Object.keys(hooks).length > 0)
        existing.hooks = hooks;
    else
        delete existing.hooks;
...
function isManagedHookEntry(entry) {
    return [entry.command, entry.bash, entry.powershell].some((value) => typeof value === 'string' && value.includes('kata-hook-guard.mjs'));
}
...
export function skillPath(platform, commandId) {
    return platformSkillPath(platform, 'project', commandId);
}
export function installationRoot(scope, options) {
    if (scope === 'project')
...
async function readManifest(root) {
    ...
    if (content === undefined)
        return { version: 1, commandManifest, files: {} };
    const parsed = JSON.parse(content);
    if (!isOwnershipManifest(parsed))
        throw new Error(`Invalid Kata adapter ownership manifest: ${path}`);
    return parsed;
}
