---
description: Run the kata-tweak Kata workflow
---

Equivalent Kata skill: `kata-tweak`
Command name: `/kata-tweak`
...
- Decide isolation, development, and review workflow choices before starting the change.
...
```bash
kata-cli tweak --change <change-id> --isolation <mode> --development <mode> --review <mode>
```
...
`/kata-tweak` owns the user-facing decision flow. Do **not** rely on CLI TTY prompts for isolation, development, or review mode; many host platforms invoke the CLI non-interactively.

Before running `kata-cli tweak`, resolve these three choices in the agent conversation:
...
Do not ask the user to run `/comet-open` manually after `/kata-open`. When `workflowProfile.comet.openStatus` is `required`, `/kata-design <task>` performs the required acknowledgement before entering `plan`. Follow the returned next action after `/kata-tweak` completes.
