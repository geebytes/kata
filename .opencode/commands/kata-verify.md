---
description: Run the kata-verify Kata workflow
---

Equivalent Kata skill: `kata-verify`
Command name: `/kata-verify`
...
3. Record what came back, unchanged — **and report how long the pass took**. Two things write as the pass proceeds, so a
   ...
   ```bash
   kata-cli adversarial note --change <task-id> --node verify --from-file <line.json>        # one line per BATCH of work
   kata-cli adversarial finding add --change <task-id> --node verify --from-file <finding.json>   # as each is confirmed
   ```
