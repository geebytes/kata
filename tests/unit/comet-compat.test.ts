...
describe('Comet compatibility boundary', () => {
    ...
    it('loads runtime compatibility from comet-compat.yaml as the single source of truth', async () => {
        ...
        // v1 inputs are upgraded to v2 schema internally (forward-compatible).
        expect(loadCometCompatibility(manifestPath)).toEqual({
            version: 2,
            minVersion: '3.4.5',
            maxVersion: '3.5.0',
            capabilities: { init: true, next: true },
            source: 'kata-bundled',
        });
    ...
    it('rejects mocked installed Comet versions using the manifest-loaded client check path', async () => {
        // The kata-bundled compat manifest declares minVersion: 0.4.0 / maxVersion: 2.0.0.
        const client = new CometClient({
            command: 'comet',
        ...
        await expect(client.assertInstalledVersion('0.3.0')).rejects.toThrow(/outside compatibility range/);
        await expect(client.assertInstalledVersion('2.0.1')).rejects.toThrow(/outside compatibility range/);
        await expect(client.assertInstalledVersion('0.4.0')).resolves.toBeUndefined();
    });
