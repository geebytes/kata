...
describe('Git Flow isolation', () => {
    ...
    it('skips git flow init with a deferred hint when the worktree has uncommitted changes', async () => {
        // Regression: previously `git flow init` walked through to git, which
        // raised "Fatal: Working tree contains unstaged changes" because init's
        // internal `git checkout -b develop` was rejected on a dirty worktree.
        const calls: string[][] = [];
        const run: GitCommandRunner = (_root, args) => {
            calls.push(args);
            const command = args.join(' ');
            if (command === 'rev-parse --is-inside-work-tree') return { ok: true, stdout: 'true' };
            if (command === 'config --get gitflow.branch.master') return { ok: false, stdout: '' };
            if (command === 'config --get gitflow.branch.develop') return { ok: false, stdout: '' };
            // Comet just wrote a batch of files; the worktree is dirty.
            if (command === 'status --porcelain') {
                return { ok: true, stdout: ' M comet-rule.md\n?? .agents/skills/comet-open/SKILL.md\n' };
            }
            return { ok: false, stdout: '' };
        };
        ...
        const result = await initializeGitFlowProject('/repo', {
            ...
            executeInteractive: async () => { interactiveCalled = true; },
        });
        ...
        expect(result.userHint).toMatch(/未提交改动.*git flow init -d/);
        expect(executeCalled).toBe(false);
        expect(interactiveCalled).toBe(false);
        // The pre-flight must NOT have tried to call `git flow` at all.
        expect(calls.find((c) => c[0] === 'flow')).toBeUndefined();
    });
