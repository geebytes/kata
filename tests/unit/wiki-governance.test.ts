...
describe('Wiki governance', () => {
    ...
    async function writePassingGates(root: string, taskId: string): Promise<void> {
        ...
        await writeFile(join(root, `.kata/tasks/${taskId}/review.json`), `${JSON.stringify({ findings: [], status: 'approved', reviewEvidence: 'Fixture simulates explicit reviewer approval.' }, null, 2)}\n`);
        await writeFile(
