...
describe('Kata platform installer', () => {
    ...
    it('synthesizes installable platform candidates when none are detected (e.g. for global scope)', async () => {
        // Regression: previously `promptInitPlan` in global scope on a fresh machine
        // received an empty `platforms` array (discoverPlatforms only returns
        // markers that already exist on disk), so the checkbox prompt failed with
        // "No selectable choices". The wizard must synthesize candidates from the
        // platform definitions so the user can actually install something.
        const home = await tempRoot('kata-empty-home-');
        const output = new PassThrough();

        const plan = await promptInitPlan([], {
            output,
            home,
            projectRoot: home,
        });

        expect(plan.scope).toBe('project'); // select() returns the first option (project) on non-TTY
        const candidateIds = plan.detected.map((p) => p.platform);
        // Every supported platform (codex, claude-code, opencode, ...) is offered.
        expect(candidateIds).toContain('codex');
