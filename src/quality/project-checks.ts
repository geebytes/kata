...
async function discoverProjectQualityChecks(root: string): Promise<CheckCommand[]> {
  ...
  return dedupe(commands).map((command) => commandLineToCheck(root, command)).filter((check): check is CheckCommand => Boolean(check));
}

async function candidateConstraintFiles(root: string): Promise<string[]> {
...
function extractAcceptanceGateCommands(markdown: string): string[] {
...
function commandLineToCheck(root: string, commandLine: string): CheckCommand | null {
...
function inferCheckName(command: string, args: string[]): string {
  if (command === 'make' && args[0]) return args[0];
  return command.replace(/[^A-Za-z0-9_.-]+/g, '-').replace(/^-|-$/g, '') || 'check';
}

function inferEvidenceKind(name: string): EvidenceKind {
...
function dedupeChecks(checks: CheckCommand[]): CheckCommand[] {
