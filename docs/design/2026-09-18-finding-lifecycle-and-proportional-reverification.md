...
> 状态：**已实现，测量已完成**（2026-09-18，平台级）——F1–F5 全部落地，§11 的未验证项已按 `2026-09-18-finding-lifecycle-measurements.md` 实测收口。
...
## 11. 未验证 / 已知边界 —— **已收口**（测量见 `2026-09-18-finding-lifecycle-measurements.md`）

| 项 | 落实方式 | 实测结果 |
|---|---|---|
| **F1 发现寿命** | `schemas/review.schema.json` + `adversarial-review.schema.json` 的四个可选字段；`kata-cli findings list\|defer\|accept\|carry`；brief / verify / archive 三处可见 | 已交付（`e872ada`），`tests/unit/finding-disposition.test.ts` 5 例 |
| **F2 逐路径摘要 + delta** | `computePathDigests` 与 `manifestHash` **并列**（后者推导未改，有逐字节回归锁）；`adversarial brief --since`；门禁机械校验覆盖（`delta_stale` / `delta_unavailable`） | 机制已交付（`7e57214`）。**`pathDigests` 成本风险关闭**：owned paths 中位数 **11**、最大 43；441/1728 是目录型；已记录的那份表 **676 条** |
| **F2 的实际节省** | pass 记录 `elapsedMs` + 覆盖前**快照上一次 pass**；`adversarial status` 报 `deltaSaving`；skill 文本写明工序 | **机制已交付，数字仍待产生**：工作区里还没有一次 delta pass（字段当天才加）。**原预测（10–22 分钟 → 2–3 分钟）未被数据支持**——见下一条 |
| **F4 的推导准确率** | `relevant-checks.ts`：变更面 → 声明该路径的验收行 → 这些行的检查；**映射不成立即回退全量**并报原因 | **已实测**：81 个任务里 79 有矩阵、**0 行无路径**、**79.5% 的 owned path 被某行声明** ⇒ 文档担心的"每改必回退"不成立；79.5% 还是回退率的**下界** |
| **F4 与 `tier`/`coveredBy` 的交互** | 定序为：先推导收窄 → 再 `tier: 'frozen'` 推迟 → 再 `coveredBy` 改道；三者的"未跑"都在报告里点名 | **已定，未实测**（本工作区 `coveredBy` 尚未使用） |
| **F5** | `reviewedPaths` + `review-scope.ts`；**未声明即视为覆盖整份 revision**（保守方向）；建议只返回、不代写 | 已交付（`c0c5ded`）。**构造上不可核实**（依赖评审者诚实），因此机制只用在保守方向 |

### 最重要的一条：F4 **不是** seal 成本的解药

实测（788 份带时长的证据）：全量套件只占一次 seal 的**中位数 4.5%**（范围 0.3%–34.2%），单检查 p50 仅 **4.2s**。
真正的大头是**项目自己声明**的 `make test`（158 次运行、均 165s、覆盖 79/81 个任务）——而平台机制（F4 推导 ✓、
`tier` ✓、`coveredBy` ✓）按设计都**服从项目声明**，没有任何推导有权把它丢掉。

**并且这个大头已经修好了**：同一份证据里，串行 `make test` 最后几次是 **569.3s / 604.0s**（09-16 / 09-13），
而现配置的 `make test-parallel` 那次是 **165.4s**（09-18）。也就是说 本文档里 619s–1331s 的一轮**不能**用来
判断当前 seal 成本。

## 附录 A：一次 dogfood 的观测（证据，不是设计输入）

> **读这一节时必须同时读 §11 的更正**：这里的 619s–1331s 是**审查者的墙钟**，不是 seal 成本；同一份证据显示
> seal 里的全量套件中位数只占 4.5%，且 `make test` 已从 569s 降到 165s（现配置为 `make test-parallel`）。
