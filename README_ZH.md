...
```mermaid
flowchart TD
    P["项目 Git 分支"] --> C["Change / 目标 + 范围"]
    C --> T["Task / 工作流控制单元"]
    T --> A["Artifacts / handoff / evidence / review / judge / wiki"]
    RG["Relation Graph / task/change/artifact 关系"] --> C
    RG --> T
    W[".llmwiki / 项目记忆"] --> T
    Q["项目质检 + CI / Reviewer + Judge"] --> T
    T --> RG
```
...
```mermaid
flowchart TD
    U["用户想法 / bug / 委托请求"] --> O["/kata-open / intake"]
    O --> D["/kata-design / 验收标准 + 约束"]
    D --> B["/kata-build / TDD 实现"]
    B -- seal --> HV["hardVerify / 写入 evidence"]
    HV --> V["/kata-verify / 新鲜度 + 验收覆盖"]
    V -- "PASS + review_gate" --> RG["用户选择 Reviewer / 平台 / 模型"]
    RG --> R["/kata-review / 评审 findings"]
    R -- "judge_gate" --> JG["用户选择 Judge / 平台 / 模型"]
    JG --> J["/kata-judge / PASS / FAIL"]
    J -- "PASS + archive_gate" --> AG["用户确认归档 / Wiki / 发布证据"]
    AG --> AR["/kata-archive / 沉淀 + 归档"]
...
```mermaid
flowchart LR
    CH["Change / 目标 / 范围"] -->|contains| T["Task / 控制单元"]
...
```mermaid
sequenceDiagram
    participant H as 高阶 agent / designer/reviewer
...
flowchart TB
    subgraph Task lifecycle
        T1[task.json / 验收标准]
        T2[current-state.json / 当前 phase]
        T3[handoffs/*.json / Context Fabric]
        T4[evidence/*.json / 检查结果]
        T5[review.json / 评审 findings]
        T6[judge.json / PASS / FAIL]
        T7[wiki candidates / 沉淀知识]
    end
